...
def _semantic_grade(
    ...
    prompt = _SEMANTIC_GRADER_PROMPT.format(
        response=response[:4000],  # Truncate to avoid context overflow
    ...
    for i, assertion_text in enumerate(assertions):
        ...
        results.append(
            AssertionResult(
                ...
                evidence=str(grade.get("evidence", "No evidence returned"))[:200],
                method="semantic",
                assertion_type="assertion",
            )
        )
...
def _format_transcript(transcript: list[dict] | None) -> str:
    ...
    lines = []
    for event in transcript[:50]:  # Limit to avoid context overflow
        etype = event.get("type", "unknown")
        data = event.get("data", {})
        if etype == "tool_use":
            lines.append(f"[TOOL_USE] {data.get('name', '?')}: {str(data.get('input', ''))[:200]}")
        elif etype == "tool_result":
            content = str(data.get("content", ""))[:200]
            lines.append(f"[TOOL_RESULT] {content}")
        elif etype == "text":
            lines.append(f"[TEXT] {str(data.get('text', data.get('content', '')))[:300]}")
